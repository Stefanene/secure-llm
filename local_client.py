...
class LocalSecureLLMClient:
    ...
    def query(self, query: str, anonymized: bool = False, use_oram: bool = False, use_pir: bool = False, 
        ...
        print(f"\n[DEBUG]\tQuery contains: '{query[:50]}...'")
        print(f"[DEBUG]\tArgs: anonymized={anonymized}, use_oram={use_oram}, use_pir={use_pir}")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"\n[CLIENT] Error: {error_data.get('error', 'Unknown error')}")
                return None
...
def main():
    ...
    for i, q in enumerate(queries, 1):
        print(f"[CLIENT] Sending query {i}/{len(queries)}")
        response = client.query(**q)
        if(response):
            print(f"Response: {response[:200]}...\n")
        else: print("\n")
    
    print("[CLIENT] All queries completed!")
